...
class Al_Ultimate_Skill_Attack:

    def __init__(self, x = 97, y = 70, face_dir = 1):
        self.frameX = 0
        self.animation_names = ['Al_UltimateSkill_Attack']
        self.images = {}
        # 각 애니메이션별로 최대 프레임 너비/높이를 저장하면 출력 크기를 통일하여 흔들림을 방지할 수 있음
        self.render_size = {}

        self.TIME_PER_ACTION = 1.7
        self.ACTION_PER_TIME = 1.0 / self.TIME_PER_ACTION
        # per-animation frame count 저장
        self.frames_per_animation = {}

        for name in self.animation_names:
            if name == 'Al_UltimateSkill_Attack':
                frames = [load_image("./Skill_impact/" + name + " (%d)" % i + ".png") for i in range(1, 15)]
    ...
    def draw(self):
        ...
        if self.face_dir < 0:
            ...
            img.draw(draw_x, draw_y, img.w * 5, img.h * 5)

    def update(self):
        # 애니메이션 길이와 프레임 타임을 소유자에서 가져와 계산
        length = self.frames_per_animation.get('Al_UltimateSkill_Attack', 1)
