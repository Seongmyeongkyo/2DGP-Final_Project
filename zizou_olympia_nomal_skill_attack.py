...
class Zizou_Olympia_Nomal_Skill_Attack:
    ...
    def __init__(self, x = 97, y = 70, face_dir = 1):
        ...
        for name in self.animation_names:
            if name == 'Zizou_Olympia_NomalSkill_Attack':
    ...
    def draw(self):
        ...
        draw_x = self.x
        if self.face_dir < 0:
            img.composite_draw(0, 'h', draw_x, self.y, 36, 36)
        else:
            img.draw(draw_x, self.y, 36, 36)

    def update(self):
        # 애니메이션 길이와 프레임 타임을 소유자에서 가져와 계산
        length = self.frames_per_animation.get('Zizou_Olympia_NomalSkill_Attack', 1)
